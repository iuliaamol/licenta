...
"plot the figure to visualize"



def plot_full_radial_figure_horizontal(my_mask_path, n_points=72, angle_deg_highlight=0, save_path=None):
    import cv2
    import numpy as np
    ...
    contour = contours[0]
    ellipse_orig = cv2.fitEllipse(contour)
    center = np.array(ellipse_orig[0])
    a, b = ellipse_orig[1][0] / 2, ellipse_orig[1][1] / 2  # Major and minor axis radii

    # Forțăm elipsa să fie orizontală
    if a < b:
        a, b = b, a  # a = axa mare, b = axa mică

    angle_rad = 0.0  # => elipsă orizontală
    ...
    ellipse_display = (tuple(center), (a * 2, b * 2), 0.0)         # elipsa trasată pe orizontală
    cv2.ellipse(overlay, ellipse_display, (0, 255, 255), 2)        # yellow: forced horizontal ellipse
    ...
    if pt_ellipse is not None:
        cv2.circle(overlay, tuple(np.round(pt_ellipse).astype(int)), 4, (0, 255, 255), -1)
        cv2.line(overlay, tuple(np.round(center).astype(int)), tuple(np.round(pt_ellipse).astype(int)), (0, 255, 255), 2)

    if pt_contour is not None:
        cv2.circle(overlay, tuple(np.round(pt_contour).astype(int)), 4, (0, 0, 255), -1)
        cv2.line(overlay, tuple(np.round(center).astype(int)), tuple(np.round(pt_contour).astype(int)), (255, 0, 0), 2)
    ...
    axs[0].set_title(f"Ellipse (horizontal) and Contour  {angle_deg_highlight}°")
    axs[0].axis("off")
    ...
    axs[1].set_title("Distance to Center (Horizontal Ellipse)")
...
plot_full_radial_figure_horizontal("../evaluation/saved_masks2/C_0033_1.RIGHT_CC.png", angle_deg_highlight=0)
